use db::postgres_db::Connection;
use repository::client_repository::Client;
use rocket::Route;
...
#[post("/", data = "<client>")]
fn create(client: Json<Client>, connection: Connection) -> Json<Client> {
    let insert = Client {
        id: None,
        ..client.into_inner()
    };
...
#[put("/<id>", data = "<client>")]
fn update(id: i32, client: Json<Client>, connection: Connection) -> Json<JsonValue> {
    let update = Client {
        id: Some(id),
        ..client.into_inner()
    };
...
#[delete("/<id>")]
fn delete(id: i32, connection: Connection) -> Json<JsonValue> {
